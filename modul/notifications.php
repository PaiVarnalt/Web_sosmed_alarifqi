...
// === Gunakan strtolower untuk konsistensi case ===
$current_user_name = strtolower(trim($_SESSION['username']));
  ...
  <div class="col-md-6 p-3 border-start border-end"> 
   ...
   <?php if ($q_notifs->rowCount() > 0): ?>
    <?php while ($notif = $q_notifs->fetch(PDO::FETCH_ASSOC)): ?>
     ...
     ?>
<a href="<?= htmlspecialchars($link) ?>" class="text-decoration-none text-body text-dark-override">
  <div class="p-3 mb-2 border rounded shadow-sm <?= !$notif['IsRead'] ? 'border-primary' : '' ?>">
  <small class="float-end"><?= date("H:i", strtotime($notif['CreatedAt'])) ?></small>
  <?= $icon ?>
  ...
  <?php endif; ?>
    ...
    <?php endwhile; ?>
   <?php else: ?>
    <div class='p-5 text-center text-muted border rounded'>Tidak ada notifikasi baru.</div>
   <?php endif; ?>
  </div>
